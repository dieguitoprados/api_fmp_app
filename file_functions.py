...
def load_timeseries_ric_yahoo(ric):
    
    directory = 'C:\\Users\\diegu\\Downloads\\' # hardcoded
    path = directory + ric + '.csv' 
    raw_data = pd.read_csv(path)
    t = pd.DataFrame()
    t['date'] = pd.to_datetime(raw_data['Date'], dayfirst=True)
    t['close'] = raw_data['Close']
    #t['close'] = raw_data.get('Close')
    t.sort_values(by='date', ascending=True)
    t['close_previous'] = t['close'].shift(1)
    t['return_close'] = t['close']/t['close_previous'] - 1
    t = t.dropna()
    t = t.reset_index(drop=True)
    # cargamos x (returns) y x_str (mensaje)
    x=t['return_close'].values
    x_str= 'Real returns of '+ric
    
    return t

def load_timeseries_ric_investing(ric):
    ...
    t= pd.DataFrame()
    t['date'] = pd.to_datetime(raw_data['Date'], dayfirst=True)
    t['close'] = raw_data['Open']
    t.sort_values(by='date', ascending=True)
    # t2['close_previous'] = t2['close'].shift(1)
    # t2['return_close'] =  t2['close']/t2['close_previous'] - 1
    t['return_close'] = raw_data['Change %']
    t = t.dropna()
    t = t.reset_index(drop=True)
    # cargamos x (returns) y x_str (mensaje)
    x=t['return_close'].values
...
def load_timeseries_bench_yahoo(benchmark):
    
    directory = 'C:\\Users\\diegu\\Downloads\\' # hardcoded    
    path1 = directory + benchmark + '.csv'
    raw_data1 = pd.read_csv(path1)
    t2 = pd.DataFrame()
    t2['date'] = pd.to_datetime(raw_data1['Date'], dayfirst=True)
    t2['close'] = raw_data1['Close']
    t2.sort_values(by='date', ascending=True)
    t2['close_previous'] = t2['close'].shift(1)
    t2['return_close'] = t2['close']/t2['close_previous'] - 1    
    t2 = t2.dropna()
    t2 = t2.reset_index(drop=True)
    # cargamos x (returns) y x_str (mensaje)
    x2=t2['return_close'].values
    x_st2r= 'Real returns of'+benchmark
    
    return t2
def load_timeseries_bench_investing(benchmark):
    ...
    t2 = pd.DataFrame()
    t2['date'] = pd.to_datetime(raw_data1['Date'], dayfirst=True)
    t2['close'] =raw_data1['Open']
    t2.sort_values(by='date', ascending=True)
    # t2['close_previous'] = t2['close'].shift(1)
    # t2['return_close'] =  t2['close']/t2['close_previous'] - 1
    t2['return_close'] = raw_data1['Change %']
    t2 = t2.dropna()
    t2 = t2.reset_index(drop=True)
    x2=t2['return_close'].values
    x_st2r= 'Real returns of'+benchmark
    ...
    def plot_histogram(x, y):
        plt.figure()
        plt.hist(x,bins=100)
        plt.title(ric+' Daily Returns')
        plt.xlabel(plot_str())
        plt.show()
        
        plt.figure()
        plt.hist(y,bins=100)
        plt.title(benchmark+' Daily Returns')
        plt.xlabel(plot_str())
        plt.show()
...
# def load_synchronised_timeseries(ric, benchmark):
    
#     # get timeseries of x and y
#     table_x = file_functions.load_timeseries(ric, source_ric)
#     table_y = file_functions.load_timeseries(benchmark, source_bench)
...
def CAPM_lin_regress(x, y, nb_decimals, ric, benchmark):
    ...
    str_title= 'Scatterplot of returns'+'\n'\
        'Linear regression | ric '+ric+' | Benchmark '+benchmark+'\n'\
        'Alpha (intercept) ' +str(intercept)\
        +' | Beta '+str(slope)+'\n' 'P_value ' + str(p_value)\
        +' | R_squared '+str(r_squared)+' | stderr '+str(std_err)
